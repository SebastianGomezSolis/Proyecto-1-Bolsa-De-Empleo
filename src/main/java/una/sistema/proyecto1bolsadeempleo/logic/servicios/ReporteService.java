...
public class ReporteService {
    ...
    public byte[] generarPdfPuestosPorMesYAnio(int mes, int anio) {
        ...
        Instant inicio = yearMonth.atDay(1).atStartOfDay(zone).toInstant();
        Instant fin = yearMonth.plusMonths(1).atDay(1).atStartOfDay(zone).toInstant();
        ...
        try (PDDocument document = new PDDocument();
            ...
            if (puestos.isEmpty()) {
                y = writeLine(content, "No se encontraron puestos en el período seleccionado.", margin, y, regular, 11);
            } else {
                int contador = 1;
                ...
                for (Puesto puesto : puestos) {
                    ...
                    if (puesto.getEmpresa() != null) {
                        y = drawField(content, margin, y, regular, "Empresa", safe(puesto.getEmpresa().getCorreo()));
                    }

                    if (puesto.getFechaRegistro() != null) {
                        y = drawField(content, margin, y, regular, "Fecha de publicación", formatter.format(puesto.getFechaRegistro()));
                    }
    ...
    private void drawFooter(PDPageContentStream content,
                            float margin,
                            float pageWidth,
                            PDType1Font italic) throws IOException {
    ...
    private float drawSectionTitle(PDPageContentStream content,
                                   float margin,
                                   float y,
                                   PDType1Font bold,
    ...
    private float drawField(PDPageContentStream content,
                            float margin,
                            float y,
                            PDType1Font font,
                            String label,
